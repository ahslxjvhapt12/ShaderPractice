...
public class FieldOfView : MonoBehaviour
{
    ...
    [SerializeField] private int _edgeResolution; // 탐색 횟수를 찾아보자
    ...
    private void DrawFieldOfView()
    {
        ...
        for (int i = 0; i < stepCount; i++)
        {
            ...
            if (i > 0)
            {
                bool edgeDistanceOver = Mathf.Abs(oldViewCastInfo.distance - info.distance) > _edgeDistanceThreshold;

                if (oldViewCastInfo.hit != info.hit || (oldViewCastInfo.hit && info.hit && edgeDistanceOver))
                {
                    var edge = EdgeFind(oldViewCastInfo, info);
    ...
    private EdgeInfo EdgeFind(ViewCastInfo minInfo, ViewCastInfo maxInfo)
    {
        float min = minInfo.angle;
        float max = maxInfo.angle;
        ...
        for (int i = 0; i < _edgeResolution; i++)
        {
            ...
            var castInfo = ViewCast(min);

            bool edgeDistanceThresholdExceeded = MathF.Abs(minInfo.distance - castInfo.distance) > _edgeDistanceThreshold;

            if (castInfo.hit == minInfo.hit && !edgeDistanceThresholdExceeded)
            {
                min = mid;
